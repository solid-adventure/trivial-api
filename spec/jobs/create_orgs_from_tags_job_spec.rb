...
RSpec.describe CreateOrgsFromTagsJob, type: :job do
  ...
  before do
    ...
    Organization.create_by_customer_id(0)
  end
  ...
  it 'creates an organization for each unassociated customer_id' do
    ...
    expect(Organization.count).to eq(1) # Only one organization created initially
    expect(Tag.where(context: 'customer_id', taggable_type: 'Organization').count).to eq(1)
